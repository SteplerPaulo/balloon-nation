...
class YearlyReport extends Formsheet{
	...
	protected static $net_income = 0;
	
	function YearlyReport(){
		$this->showLines = !true;
		$this->FPDF(YearlyReport::$_orient, YearlyReport::$_unit,array(YearlyReport::$_width,YearlyReport::$_height));
		$this->createSheet();
	}
	...
	function dtls($data,$page){
		...
		foreach($data as $itm){
			...
			YearlyReport::$net_income+=$profit;
			$y++;
		}
		//$y++;
	...
	function net_income(){
		...
		$this->section($metrics);
		$this->GRID['font_size']=23;
		$this->centerText(0,5,'TOTAL NET INCOME: '.number_format(YearlyReport::$net_income,2),40,'');
	}
